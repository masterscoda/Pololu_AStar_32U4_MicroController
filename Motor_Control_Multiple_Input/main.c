 ...
 * Scott Bossard
 * 2/21/2108
 * Motor control using multiple inputs
 */
...
int speed = 10; // used for duty cycle default 10%
void handleInput();
...
void handleInput() {
		...
		for (j =0; j <= recv_buffer_ptr; j++ ){
			switch(recv_buffer[j]) {
				case 'w':
					sprintf( outputBuffer,"Motor forward 360 degrees.\r\n");
					OnMotor2();
					PORTC |= ( 1 << PORTC7 );// Yellow LED toggle on
					global_counts_m2 = 0; //reset to go 360 degrees forward from current position
					while (global_counts_m2 < 2249){
						//One revolution = 2249 counts
						motorForward();
					}
					OffMotor2();
					PORTC &= ~( 1 << PORTC7 );// Yellow LED toggle off
					break;
				case 's':
					sprintf( outputBuffer,"Motor backward  360 degrees.\r\n");
					OnMotor2();
					PORTD &= ~( 1 << PORTD5 );// Green LED toggle on
					global_counts_m2 = 0; //reset to go 360 degrees forward from current position
					while (global_counts_m2 > -2249){
						//One revolution = -2249 counts
						motorBackward();
					...
					OffMotor2();
					PORTD |= ( 1 << PORTD5 );// Green LED toggle off
					break;
				case 'd':
